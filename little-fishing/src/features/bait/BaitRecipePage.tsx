...
export function BaitRecipePage({ isFishing, onSaved }: { isFishing: boolean; onSaved: () => Promise<void> }) {
  ...
  return <section className="section-page">
    ...
    <div className="bait-layout"><article className="paper-card"><label className="field-label">已保存方案<select value={data.recipeId} disabled={isFishing} onChange={(event) => void chooseRecipe(Number(event.target.value))}>{data.recipes.map((recipe) => <option value={recipe.id} key={recipe.id}>{recipe.name}</option>)}</select></label><label className="field-label bait-name-field">配方名称<input value={name} maxLength={24} disabled={isFishing} onChange={(event) => setName(event.target.value)} /></label><div className="ingredient-list">{data.ingredients.map((ingredient) => {
    ...
    })}</div></article><aside className="paper-card bait-preview"><h3>本次配方</h3><p>已选 {entries.length} 种成分 · 当前填写总量 {total.toFixed(1)} 份</p><BaitFlavorRadar flavor={currentFlavor} /><p className="hidden-rule-note">五维属性按当前配比归一化计算，最大值为 1。鱼类每天变化的偏好与实际匹配度仍保持隐藏。</p><div className="bait-save-actions"><button className="primary-button" disabled={isFishing || total <= 0 || !name.trim()} onClick={() => void save(false)}>{data.recipeId === 1 ? "保存为新方案" : "保存当前方案"}</button><button className="quiet-button" disabled={isFishing || total <= 0 || !name.trim()} onClick={() => void save(true)}>另存为新方案</button></div>{message && <div className="error-strip" role="status">{message}</div>}</aside></div>
  </section>;
}
