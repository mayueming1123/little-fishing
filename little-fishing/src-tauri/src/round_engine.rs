...
const STATUS_TEXTS: [&str; 30] = [
    ...
    "浮标轻晃两下站稳，像是在说已经准备好了。",
    "鱼饵开始散味，水下会不会买账只能慢慢看。",
    "钓线已经垂进水里，这一竿正式进入勿扰模式。",
    "浮标轻轻点头，像是接受了今天的值班安排。",
    "水面替你收好了鱼钩，接下来只管慢慢等。",
    "饵团稳稳落位，水下的试吃环节尚未开放。",
    "竿稍放松下来，今天的故事从一片安静开始。",
    "钓组已经入水，鱼和运气正在各走各的路。",
    "浮标在水面露出一点红色，耐心也跟着上线了。",
    "这一竿落得很轻，连水波都没有急着表态。",
    "鱼饵带着复杂配方沉下去，等一位有缘食客。",
    "岸边重新安静，浮标接过了接下来的工作。",
];

const AMBIENT_EVENTS: [&str; 30] = [
    "风从芦苇那边绕过来，水面多了几道细纹。",
    ...
    "远处的云慢慢拉长，给水面换了一层柔和的颜色。",
    "几粒草籽被风送进水里，沿着岸边排成一小队。",
    "阳光从树叶间漏下来，在鱼线上停了片刻。",
    "一片枯叶落在脚边，声音轻得差点被水声盖住。",
    "岸边泥土被晒出淡淡暖意，空气里没有一点催促。",
    "远处有人咳嗽一声，很快又只剩下风吹水面。",
    "树梢轻轻摇晃，影子在水里慢了半拍才跟上。",
    "一小阵风卷走饵盆旁的碎屑，顺便打扫了岸边。",
    "天色短暂暗了一点，随后云层把阳光又还了回来。",
];

const WATER_EVENTS: [&str; 30] = [
    ...
    "浮标忽然向下顿了半格，下一秒又若无其事地浮起。",
    "鱼线旁荡开一圈小水纹，像有什么在下面拐了个弯。",
    "几颗细小气泡贴着钓线升起，到水面后悄悄破开。",
    "浮标左右摇了三次，最后把原因归给了水流。",
    "水下传来一阵很轻的拖拽感，却没有继续发展。",
    "一道波纹从远处跑来，碰过浮标后分成了两半。",
    "浮标露出的部分少了一点，很快又恢复原来的高度。",
    "鱼线在水中轻弹了一下，像收到一封没有正文的回信。",
    "水面有东西迅速闪过，留下的只有一小串碎泡。",
    "浮标慢慢转过一圈，似乎完成了一次例行巡查。",
];

const TACKLE_EVENTS: [&str; 30] = [
    ...
    "鱼钩勾住一片厚叶，抖落以后重新沉回原来的位置。",
    "主线在导环边轻轻磨响，调整角度后重新安静下来。",
    "铅坠卡进软泥里，缓慢提线才把它完整带了出来。",
    "一小段旧鱼线缠上钓组，解开后顺手把它收进了垃圾袋。",
    "浮标被水草推得侧躺，清理周围后又重新站直。",
    "竿架松了一点，扶正拧紧以后继续稳稳托住鱼竿。",
    "抛投落点偏了半米，收回重抛后终于回到理想位置。",
    "饵料在入水时散掉一角，补饵后钩尖重新藏得严实。",
    "子线绕上铅皮座，耐心分开以后没有留下折痕。",
    "线轮泄力拧得太松，轻调两格后重新守住这一竿。",
];

const WILDLIFE_EVENTS: [&str; 30] = [
    ...
    "一只翠鸟停在斜枝上，看了一会儿又换到更好的座位。",
    "两条小鱼追着一片面包屑游过，对鱼钩保持了职业距离。",
    "一只小螃蟹搬着砂砾路过，工作效率比浮标高得多。",
    "水蛇从远处安静游过，很快消失在另一片芦苇后面。",
    "一只豆娘落上鱼竿，透明翅膀在阳光下闪了一下。",
    "岸边的小鸟叼走一根草茎，没忘记看一眼空空的鱼篓。",
    "一群蝌蚪贴着浅水转弯，整齐得像提前排练过。",
    "一只乌龟在远处露出脑袋，确认安全后又沉了下去。",
    "水黾围着浮标绕了一圈，像在测量这片临时领地。",
    "一条小鱼追虫冲出水面，用很响的一声证明鱼确实在附近。",
];

const STORY_EVENTS: [&str; 30] = [
    ...
    "浮标轻点一下，像水下有人按了门铃却不愿留下姓名。",
    "一串气泡围着鱼线排成问号，答案仍旧沉在下面。",
    "风把浮标推远一点，又像想起什么似的把它送了回来。",
    "水面反光正好落在浮标顶端，它短暂冒充了一座小灯塔。",
    "鱼线忽然动了一下，像有谁在水下翻身时碰到了桌角。",
    "一片叶子绕浮标转了两圈，最终没有找到停车的位置。",
    "几道波纹从不同方向赶来，在钓点开了场没有结论的会议。",
    "浮标微微倾斜，像在偷听水下那场不对外公开的讨论。",
    "水草从钓线旁慢慢经过，带着一副知道内幕却不肯说的样子。",
    "远处一声鱼跃格外清楚，像中奖号码开在了隔壁桌。",
];
...
mod tests {
    ...
    #[test]
    fn every_event_category_has_thirty_descriptions() {
        assert_eq!(STATUS_TEXTS.len(), 30);
        assert_eq!(AMBIENT_EVENTS.len(), 30);
        assert_eq!(WATER_EVENTS.len(), 30);
        assert_eq!(TACKLE_EVENTS.len(), 30);
        assert_eq!(WILDLIFE_EVENTS.len(), 30);
        assert_eq!(STORY_EVENTS.len(), 30);
        assert_eq!(event_description_seeds().len(), 180);
    }
